const bookingExtraServiceService = require('../services/bookingExtraServiceService');
...
class BookingExtraServiceController {
  async createBookingExtraService(req, res) {
    try {
      ...
      res.status(error.statusCode || 500).json({ message: error.message || 'Failed to create booking extra service', error: error.message });
    }
  }
  ...
  async deleteBookingExtraService(req, res) {
    try {
      ...
      res.status(error.statusCode || 500).json({ message: error.message || 'Failed to delete booking extra service', error: error.message });
    }
  }
}
