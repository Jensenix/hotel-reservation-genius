...
class RoomAvailabilityService {
  ...
  async getRoomAvailability(date) {
    ...
    const roomTypes = await RoomType.findAll({
      include: [{ model: Room, as: 'rooms', attributes: ['id', 'roomNumber', 'status', 'floor'] }],
      order: [['basePrice', 'ASC']]
    });
    ...
    const availabilityData = await Promise.all(roomTypes.map(async (roomType) => {
      ...
      const availableRooms = roomType.rooms.filter(r => !bookedRoomIds.includes(r.id) && r.status === 'available').length;
      const occupiedRooms = roomType.rooms.filter(r => bookedRoomIds.includes(r.id) || r.status === 'occupied').length;
    ...
    overall.availabilityRate = overall.totalRooms > 0 ? Math.round((overall.availableRooms / overall.totalRooms) * 100) : 0;
    overall.occupancyRate = overall.totalRooms > 0 ? Math.round((overall.occupiedRooms / overall.totalRooms) * 100) : 0;

    return { date: targetDate.toISOString().split('T')[0], overall, byRoomType: availabilityData };
  }
  ...
  async getRoomTypeAvailability(roomTypeId, date) {
    const roomType = await RoomType.findByPk(roomTypeId, {
      include: [{
        model: Room, as: 'rooms', where: date ? {} : undefined,
        include: [{
          model: Booking, as: 'bookings', required: false,
          where: date ? {
            status: { [Op.in]: ['confirmed', 'checked_in'] },
            [Op.or]: [
              { checkInDate: { [Op.lte]: new Date(date) }, checkOutDate: { [Op.gt]: new Date(date) } },
              { checkInDate: { [Op.lte]: new Date(date) }, checkOutDate: { [Op.gte]: new Date(date) } }
            ]
          } : undefined
        }]
      }]
...
module.exports = new RoomAvailabilityService();