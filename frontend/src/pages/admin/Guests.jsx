...
export default function Guests() {
  ...
  const initialFormState = { fullName: '', email: '', password: '', phoneNumber: '', role: 'guest' };
  ...
  const { state, actions } = useAdminCRUD({ endpoint: 'users', initialFormState, mapApiResponse });

  const finalFilteredUsers = state.filteredData.filter(user => !roleFilter || user.role === roleFilter);
        ...
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => actions.setShowModal(true)} className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-xl border-0 px-8 py-3 rounded-xl font-semibold transition-all duration-300">
          ...
          <GuestTable 
            ...
            onDelete={(user) => { actions.setDeleteTarget(user); actions.setShowDeleteModal(true); }}
