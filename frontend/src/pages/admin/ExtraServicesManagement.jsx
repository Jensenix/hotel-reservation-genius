...
import { Plus, Coffee, Car, Utensils, Bell, Gift, Edit2, Trash2, Search } from 'lucide-react';
...
const ExtraServicesManagement = () => {
  ...
  const handleSubmitService = async (e) => {
    ...
    try {
      if (editingService) {
        await apiService.extraServices.update(editingService.id, serviceFormData);
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50/20 to-slate-50">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <span className="text-amber-600 text-xs font-semibold tracking-widest uppercase">Extra Services</span>
                ...
                <h1 className="text-5xl font-light text-slate-800 mb-2 tracking-tight">
                  Hotel <span className="font-semibold text-amber-600">Services</span>
                </h1>
                <p className="text-slate-500 text-sm tracking-wide">Manage Additional Services & Amenities</p>
              </div>
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl shadow-2xl border border-slate-700 p-8 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-slate-400 text-xs uppercase tracking-wider mb-2">Total Services</p>
            ...
            <div className="bg-gradient-to-br from-emerald-900 to-emerald-800 text-white rounded-2xl shadow-2xl border border-emerald-700 p-8 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-emerald-300 text-xs uppercase tracking-wider mb-2">Active Services</p>
                <p className="text-4xl font-light">{services.length}</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-900 to-blue-800 text-white rounded-2xl shadow-2xl border border-blue-700 p-8 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-4xl font-light">{Math.ceil(services.length * 0.4)}</p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredServices.map((service) => (
                <div key={service.id} className="bg-white rounded-2xl shadow-xl border border-slate-200 hover:shadow-2xl transition-all duration-500 overflow-hidden group">
                    ...
                    <div className="relative">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-white text-2xl font-light">{formatCurrency(service.price)}</p>
                        </div>
                      </div>
                      <h3 className="text-2xl font-light text-white mb-3 tracking-tight">{service.name}</h3>
                      <p className="text-slate-400 text-sm leading-relaxed">{service.description || 'No description'}</p>
                    </div>
                  </div>
        ...
        >
          <form onSubmit={handleSubmitService} className="space-y-6">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Service Name</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Price ($)</label>
              <input
                type="number"
                required
                min="0"
