...
const OurRooms = () => {
  ...
  useEffect(() => {
    if (socket) {
      socket.emit('join_room', 'room:public_availability');

      return () => {
        socket.emit('leave_room', 'room:public_availability');
      };
    }
  }, [socket]);
  ...
  const handleSearchDates = () => {
    if (tempCheckIn && tempCheckOut) {
      ...
      if (diffDays <= 0) {
        ...
        finalCheckOut = getLocalYYYYMMDD(checkOutDate);
        alert(
          `Bookings are limited to a maximum of ${MaxStayDays} nights. Your dates have been adjusted.`,
        );
      }
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 animate-in fade-in zoom-in duration-300">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 mt-2">
                Enter your dates to see real-time room availability and exact
                pricing.
              </p>
            </div>
