...
export default function GuestTable({ users, loading, allUsers, onEdit, onDelete, onAddUser }) {
  const getRoleColor = (role) => role === 'admin' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-blue-100 text-blue-700 border-blue-200';
  const getRoleIcon = (role) => role === 'admin' ? <Shield className="w-4 h-4" /> : <User className="w-4 h-4" />;
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
          ...
          <tbody className="divide-y divide-slate-100">
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-amber-50/50 transition-colors duration-200">
                ...
                <td className="px-8 py-5 text-slate-600"><Mail className="inline w-4 h-4 mr-2 text-slate-400" />{user.email}</td>
                <td className="px-8 py-5 text-slate-600">
                  {user.phoneNumber ? <><Phone className="inline w-4 h-4 mr-2 text-slate-400" />{user.phoneNumber}</> : '-'}
                </td>
                <td className="px-8 py-5">
                  <div className={`inline-flex items-center px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider border ${getRoleColor(user.role)}`}>
                    {getRoleIcon(user.role)} <span className="ml-2">{user.role}</span>
                  </div>
                </td>
                <td className="px-8 py-5">
                  <div className="flex items-center justify-center gap-3">
                    <Button onClick={() => onEdit(user, { ...user, password: '' })} className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white border-0 px-4 py-2 text-sm rounded-lg shadow-md transition-all">
      ...
      {users.length === 0 && (
        <div className="text-center py-16">
          ...
          <Button onClick={onAddUser} className="bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg border-0 px-8 py-3 rounded-xl font-semibold">
